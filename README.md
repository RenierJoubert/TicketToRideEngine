...
For my personal project I wish to program an application based on the board game *Ticket to Ride*, more specifically this application will take a list of *tickets* and the *gameMap*, where a *ticket* is a subclass of a *route* from the *gameMap* and map that list to the optimized *path*, that is the *path* which yeilds the **maximum number of points**. To elaborate, in the game a player recieves an arbitrary number of cards each game, where each card represents a ticket that in essence is a route from city A -> city B needing to be completed and a point value associated with that route, as well as a distance between the two cities. If completed the player earns x points, if not completed x points are deducted from their total score. So, for this application I will be working at a higher level of abstraction than the actual board game as I will not be touching on several other intricacies of the game, however the basic premise of optimization will still serve functional in actual play despite certain oversights made.
